...
def CRIV_distrib(param, criv = './CRIV.csv', param_distrib = './param_distrib.csv', sutra_inp_table = 'param_table.csv',  
	sutra_inp_file = 'param.txt', q_riv = 'river_budget.txt', aq_head = 'head_cell_aq.csv', criv_value = '../output/CRIV_distrib.csv'):
	#---  Geometry - User input
	ct = param['ct'] # cell thickness [m]
	cw = param['cw'] # cell width [m]
	w = param['w'] # river width [m]
	...
	np.savetxt(param_distrib,output,delimiter=' ')
        #tested parameter
	Param = h_left     
	#output file
	param_output = criv
	...
	with open(param_distrib,'r') as param_gen:  
		...
		with open(sutra_inp_table,'w') as output:
			...
			for i in Param:
				...
				float(row[4]), float(row[5]), float(s1), float(s2),
				float(row[6]), float(row[7]), float(row[8]), float(Kvb),
				float(i),float(h_riv),float(h_right),calcXfar )) 
		...
		with open(sutra_inp_table) as csv_data:
			...
			#	try :		    
				...
				data1 = np.genfromtxt(criv_distrib, dtype=[('Q',float),('param',float)], delimiter = ' ')
				Q = data1['Q']
				param = data1['param']
				#stat
				x = param[:,np.newaxis]
				...
				criv_file.close()
				#except:
				   # print('error on input param set')
